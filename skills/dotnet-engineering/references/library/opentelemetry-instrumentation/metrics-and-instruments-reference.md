> **Library reference.** Part of the `dotnet-engineering` skill. The rules in `SKILL.md` and in
> `references/doctrine/` take precedence over anything written here; where they disagree, see
> `references/doctrine/precedence.md`. Adapted from `dotnet-skills` by Aaron Stannard (MIT) — see `NOTICE.md`.
...
This is a companion to [README.md](README.md). See it for core principles, the "System.Diagnostics-first" architecture, metrics naming conventions, and metric recording method naming.
